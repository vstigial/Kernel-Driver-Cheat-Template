...
extern "C" {
	NTKERNELAPI NTSTATUS IoCreateDriver(PUNICODE_STRING DriverName,
										PDRIVER_INITIALIZE InitializiationFunction);
...
namespace driver {
	...
	// Holds data we send between user mode & kernel mode
	struct Request {
		HANDLE process_id;
...
NTSTATUS driver_main(PDRIVER_OBJECT driver_object, PUNICODE_STRING registry_path) {
	...
	RtlInitUnicodeString(&device_name, L"\\Device\\DriverName");
	...
	RtlInitUnicodeString(&symbolic_link, L"\\DosDevices\\DriverName");
	...
	debug_print("Established driver symbolic link.\n");
	...
	debug_print("Finished initializing.\n");

	return status;
}

// kdmapper sends NULL parameters to DriverEntry function, so we create a driver object manually
NTSTATUS DriverEntry() {
	debug_print("Starting entry...\n");

	UNICODE_STRING driver_name = {};
	RtlInitUnicodeString(&driver_name, L"\\Driver\\DriverName");
